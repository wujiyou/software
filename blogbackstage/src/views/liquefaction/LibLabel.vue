<template>
  <div>
    ...
    <el-row>
      <el-col :span="24">
        <h5
          style="border-bottom:1px solid #ece8e8;padding-bottom:15px;padding-left:15px"
        >
          查询信息
        </h5>
      </el-col>
...
<script>
import { getLibLabel,postAppId,postTuckComb,getSearchLabel } from "../../utils/api";
export default {
  data() {
    return {
      wsr: "",
  ...
  methods: {
    ...
    sendLabel(obj) {
       var rt = 0;
        ...
        try {
            ...
            postAppId(objs).then(res=>{
              console.log("---------")
               console.log(res)
               if(res.data<1){
                 postTuckComb(objs,obj).then(val=>{
                   console.log(val)
                     let regId = val.regId;
                     let gasId = val.gasId;
                     let unit = val.unit;
                     let buildingUser = val.buildingUser;
                     let checkStatus = val.status;
                     let times = val.times;
                     console.log(regId,gasId,unit)
                       rt = this.wsr.ws_openPort(port); 
                      rt = this.wsr.ws_verifyKey(port, key, 0);
                                ...
                                if (buildingUser.length > 32) {
                                    ...
                                    buildingUser = unit.substring(64)
                                ...
                                rt = this.wsr.ws_writeBlock(port, 52, times)
   ...
   //初始化请求表格数组
   getLibLabelList() {
      getLibLabel(this.page,this.rows).then((res) => {
   ...
   search(){
     ...
     getSearchLabel(this.page,this.rows,obj).then(res=>{
       this.libLabelList=res.data.list
       console.log(res)
