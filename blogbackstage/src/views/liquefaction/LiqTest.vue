...
import {getLiqReport,
        ...
        getDownFile,
        getLiqInspection,
        postLiqSaveInspection,
        getLiqSaveFileInspection,
        getDownFileInspection,
        getLiqAnnulment,
        postLiqSaveAnnulment,
        getLiqSaveFileAnnulment,
        getDownFileAnnulment} from "../../utils/api"
export default {
  name: "report",
  data() {
    return {
  ...
  methods: {
    ...
    baoEdituser(userss) {
      ...
      getLiqAnnulment(this.gasId).then(res=>{
        console.log(res);
         if (res.code === 201) {
          this.$message({
            type: "warning",
    ...
    // 判废报告保存到数据库
     SaveUserrfei() {
       postLiqSaveAnnulment(this.feiUserForm).then(res=>{
          console.log(res);
           if (res.code == 0) {
          this.$message({
            type: "success",
    ...
    EditUserfei() {
      ...
      getLiqSaveFileAnnulment(this.reportNumber).then(res=>{
         console.log(res);
          if (res.code == 200) {
        // 访问接口获取成功以后再改变状态
    ...
    async SaveUserfei() {
      ...
      getDownFileAnnulment(this.regId).then(res=>{
   ...
   dingEdituser(users) {
      ...
      getLiqInspection(this.gasId).then(res=>{
         if (res.code === 201) {
           this.$message({
             type: "warning",
             message: "请先填写检验报告",
    ...
    // 定检报告保存到数据库
     rePort() {
      // console.log(1);
      postLiqSaveInspection(this.reportUserForm).then(res=>{
        console.log(res);
         if (res.code == 0) {
          this.$message({
             type: "success",
    ...
    // 定检报告保存文件
    EditUserreport() {
      getLiqSaveFileInspection(this.reportNumber).then(res=>{
         console.log(res);
        if (res.code == 200) {
        // 访问接口获取成功以后再改变状态
        // this.loadingbut = false;
    ...
    // 定检报告下载文件
    SaveUserreport() {
      getDownFileInspection(this.gasId).then(res=>{
          const blob = new Blob([res], {
           type: "application/vnd.ms-excel",
         });
    ...
    SaveUser() {
      // console.log(1);
      getDownFile(this.gasId).then(res=>{
        const blob = new Blob([res], {
        type: "application/vnd.ms-excel",
      });
      ...
      this.$refs.editUserForm.validate( (valid) => {
        if (valid) {
          console.log(this.editUserForm);
           getLiqSave(this.editUserForm).then(res=>{
              console.log(res);
             if (res.code == 200) {
               this.$message({
                 type: "success",
    ...
    //  检测报告保存文件
     EditUser() {
      ...
      this.loadingbut = true;
      this.loadingbuttext = "加载中...";
      getLiqSaveFile(this.gasId).then(res=>{
        console.log(res)
         if (res.code == 200) {
        // 访问接口获取成功以后再改变状态
        this.loadingbut = false;
    ...
    async showEdituser(user) {
      ...
      this.dialogFormVisibleEdit = true;
      // console.log(user.gasId);
      getLiqDetection(user.gasId).then(res=>{
         console.log(res);
        if (res.code == 200) {
    ...
    async search() {
      ...
      getSearchReport(this.currpage,this.pageSize,params).then(res=>{
        console.log(res);
    ...
    handleCurrentChange(val) {
     ...
     getLiqReportList() {
       getLiqReport(this.currpage,this.pageSize).then(res=>{
          this.liqReportList = res.data.list;
          this.total = res.data.totalCount;
          console.log(res);
