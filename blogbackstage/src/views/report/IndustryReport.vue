  ...
  <div>
    <el-card class="box-card" style="margin:20px 20px 0 20px;">
      ...
      >
        ...
        <el-table-column prop="medium" label="介质" show-overflow-tooltip></el-table-column>
      ...
      >
        <el-form :model="editUserForm" :rules="rules" ref="editUserForm">
          ...
          <el-col :span="24">
            <el-col :span="8">
              <el-form-item class="red_s" label="原始容积" label-width="140px" prop="volume">
...
<script>
import $ from "jquery";
import {getIndustryReport} from "../../utils/api"
export default {
  name: "report",
  data() {
    return {
      ...
      rules: {
        ...
        Upper: [
          { required: true, message: "上部壁厚（mm）不能为空", trigger: "blur" }
        ],

        Mid: [
          { required: true, message: "中部壁厚（mm）不能为空", trigger: "blur" }
        ],
        Bottom: [
          { required: true, message: "下部壁厚（mm）不能为空", trigger: "blur" }
        ],
        Weight: [
  ...
  methods: {
    ...
    async Registration() {
      ...
      console.log(res);
      const sty1 = res.data.toString();
      this.editUserForm.ReportNumber = sty1;
    },
    // 判废报告弹框
    async baoEdituser(userss) {
      this.gasId = userss.gasId;
      const res = await this.$http.post(
        `/industrial/no/reminder/no/${this.gasId}`
      );
      console.log(res);
      // debugger;
    ...
    async EditUserfei() {
      const res = await this.$http.post(
        `/industrial/no/NoOut?regId=${this.feiUserForm.regId}`
      );
      console.log(res);
      if (res.data.code == 200) {
        // 访问接口获取成功以后再改变状态
    ...
    async SaveUserfei() {
      const response = await this.$http.get(
        `/industrial/download/no?regId=${this.feiUserForm.regId}`,
        {
          responseType: "blob"
        }
      );
    ...
    async dingEdituser(users) {
      ...
      const res = await this.$http.post(
        `industrial/yes/reminder/yes/${this.gasId}`
      );
      console.log(res);
      if (res.data.code == 200) {
    ...
    async EditUserreport() {
      const res = await this.$http.post(
        `/industrial/yes/YesOut?regId=${this.reportUserForm.regId}`
      );
      console.log(res);
      if (res.data.code == 200) {
        // 访问接口获取成功以后再改变状态
    ...
    async SaveUserreport() {
      const response = await this.$http.get(
        `/industrial/download/yes?regId=${this.reportUserForm.regId}`,
        {
          responseType: "blob"
        }
      );
    ...
    async SaveUser() {
      // console.log(1);
      const response = await this.$http.get(
        `/industrial/download/inspection?gasId=${this.idI}`,
        { responseType: "blob" }
      );
      console.log(response);
    ...
    async SaveUserku() {
      this.$refs.editUserForm.validate(async valid => {
        if (valid) {
          ...
          const res = await this.$http.post(
            `/industrial/Inspector/detection/insert`,
            this.editUserForm
          );
          console.log(res);
          if (res.data.code == 200) {
    ...
    async EditUser() {
      ...
      const res = await this.$http.get(
        `/industrial/Inspector/detection?gasId=${this.idI}`
      );
      console.log(res);
      if (res.data.code == 200) {
        // 访问接口获取成功以后再改变状态
    ...
    async showEdituser(user) {
      ...
      this.dialogFormVisibleEdit = true;
      const res = await this.$http.get(
        `/industrial/Inspector/detection/show?gasId=${user.gasId}`
      );
      console.log(res);
      if (res.data.code == 200) {
        ...
        const str2 = user.volume.toString();
        this.editUserForm.volume = str2;
    ...
    async search() {
      const res = await this.$http.get(
        `/cylinder/industrial/select?pageSize=${this.pageSize}&currpage=${this.page}&gasId=${this.gasId}&reportStatus=${this.reportStatus}&appId=${this.appId}&medium=${this.medium}&buildingUser=${this.buildingUser}`
      );
      console.log(res);
      this.industryReportList = res.data.data.list;
